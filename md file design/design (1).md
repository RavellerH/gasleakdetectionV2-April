...
| 2 | Setup pin kontrol | Set `DRDY`, `RESET`, `SYNC`, dan `CS`; lalu idle-kan `CS` ke `HIGH` | `pinMode(PIN_DRDY, INPUT); digitalWrite(PIN_CS_ADS, HIGH);` |
| 3 | Manual reset ADS | Reset ADS secara manual sebelum inisialisasi | `digitalWrite(PIN_RESET, LOW); delay(100); digitalWrite(PIN_RESET, HIGH); delay(100);` |
...
| Gain / TCXO flag | `0` | argumen terakhir dipakai `0` pada runtime reference |
...
| Flow | Inti langkah | Contoh call |
|---|---|---|
| Startup LoRa | Mulai `SPI_LORA`, inisialisasi radio dengan frekuensi / bandwidth / SF / CR target | `SPI_LORA.begin(...); radio.begin(id.loraFreq, 125.0, 9, 7, RADIOLIB_SX127X_SYNC_WORD, 17, 8, 0);` |
...
```cpp
enum NullingRequestSource {
    NULLING_SOURCE_BOOT,
    NULLING_SOURCE_MQTT,
    NULLING_SOURCE_LORA,
    NULLING_SOURCE_SERIAL,
    NULLING_SOURCE_DRIFT_DETECTION,
    NULLING_SOURCE_MODBUS,
    NULLING_SOURCE_BUTTON
};

enum NullingReason {
    NULLING_REASON_BOOT,
    NULLING_REASON_BEFORE_DATASET,
    NULLING_REASON_BEFORE_INFERENCE,
    NULLING_REASON_DRIFT_CORRECTION,
    NULLING_REASON_MANUAL,
    NULLING_REASON_PERIODIC
};
...
```jsonc
{
  ...
  "run_nulling_before_start": true,      // jalankan nulling dulu bila diminta
  "save_nulling_profile": true,          // simpan profile nulling hasil terbaru
...
-> run NullingService if requested or if no valid NullingProfile exists
-> apply latest nulling profile
...
-> ADS1256 AGC
-> convert to voltage
-> optional moving average after gain compensation
...
Aturan auto transmit yang terbukti di lapangan. 
...
- threshold confidence global desain dasar dapat tetap dikonfigurasi
- untuk behavior referensi node, nilai `0.80` dianggap prioritas
...
#### 6.3.10 LoRa Nulling Command During Inference
Source: BaseDesign.

Saat inference berjalan, perintah nulling dari LoRa tidak boleh dieksekusi sembarangan. Policy target:

```text
- only valid in external power mode
- reject when alarm active unless explicitly forced by safe operator policy
- reject when dataset recording active
- if accepted, inference path pauses safely, runs NullingService, then resumes
```
...
Catatan:
- baseline build target mengikuti `ESP32-S3` tunggal ala `DESIGN_v1_3.md`
- multi-board layout lama tetap relevan hanya sebagai referensi migrasi
...
Wi-Fi + MQTT hanya dipakai pada `Dataset Stage` saat external power untuk menerima command, mengirim stream sensor, status sesi, dan ringkasan akhir.

### 10.2 MQTT Topics from ESP32 Perspective
Source: BaseDesign.

Pada `Dataset Stage`, behavior MQTT dari sudut pandang ESP32 adalah:

- subscribe command dataset / service saat external power
...
|---|---|---|
| `gas-leak-detector/{device_id}/cmd` | Subscribe | Menerima command service umum seperti start nulling, status request, atau helper command lain saat external power |
...
```json
{
  ...
  "run_nulling_before_start": true,
  "save_nulling_profile": true,
  "use_fan_intake": true,
  "fan_on_ms": 1000,
...
- Wi-Fi dan MQTT hanya boleh aktif pada `Dataset Stage`
- Wi-Fi dan MQTT tidak boleh aktif di `battery mode`
- saat node masuk `Inference Stage`, jalur Wi-Fi / MQTT harus dianggap nonaktif
- format topic `gas-leak-detector/{device_id}/...` adalah format base design yang menjadi acuan utama
- MQTT manager adalah jalur kontrol dan publish utama untuk `Dataset Stage`
...
AND BATMON = tidak ada tegangan bermakna
...
- `PG24 fault` tanpa `BATMON` berarti tidak ada sumber daya valid
...
| Stage | External 24V | Battery |
|---|---|---|
| Nulling | Allowed | Not allowed |
| Dataset | Allowed | Not allowed |
| Inference | Allowed | Allowed |
| Modbus | Allowed | Not active |
| MQTT Dataset Control | Allowed | Not used |
| LoRa Transmission | Allowed | Allowed |
| Alarm Lamp/Buzzer | Allowed | Power-aware |
...
| 28 | ANOMALY_ACTIVE_SENSOR_COUNT | count | R | Number of deviating sensors |
...
Command codes target:

| Code | Command | Allowed Power Mode | Description |
|---:|---|---|---|
| 0 | NONE | Any | No command |
| 1 | START_NULLING | External only | Start nulling stage |
| 2 | START_DATASET | External only | Start dataset generation |
| 3 | START_INFERENCE | External only | Start inference stage |
| 4 | RESET_ALARM | External only | Reset/acknowledge alarm |
| 5 | SAVE_CONFIG | External only | Save config/profile |
| 6 | REBOOT | External only | Reboot MCU |
| 7 | CLEAR_ERROR | External only | Clear error flags |
...
-> jika external power tersedia dan wiper profile belum valid: jalankan Nulling Service
-> simpan hasil nulling ke EEPROM
...
## 16. LoRa Compact Payload, Scheduling, and ACK Policy

### 16.1 Combined Design Decision

Untuk efisiensi bandwidth, arah implementasi final adalah **compact binary payload**. Node hanya mengirim `deviceId`; `siteId` dan GPS dipetakan di sisi server.

```text
Primary node identity : deviceId
siteId                : not used on LoRa payload
GPS in payload        : not used
Location mapping      : server-side deviceId -> site/location
```


### 16.2 Packet Families
Source: BaseDesign.

```text
NORMAL telemetry : packet periodik, ringkas, tanpa sensor detail
ALARM event      : packet alarm dengan snapshot sensor ringkas
ACK              : balasan untuk packet yang butuh ACK
HEALTH           : packet opsional untuk error / health status
```

Representasi tipe packet:

```cpp
enum LoRaPacketType : uint8_t {
    LORA_PKT_NORMAL = 0x01,
    LORA_PKT_ALARM  = 0x02,
    LORA_PKT_ACK    = 0x03,
    LORA_PKT_HEALTH = 0x04
};
```

### 16.3 Compact Payload Shapes

Format target dari `BaseDesign`:

```text
Normal telemetry
- 12 bytes
- fields: packetType, deviceId, seq, powerMode, aiClass, confidence, finalStatus, flags, batteryMv

Alarm event
- 32 bytes
- fields: compact header + nullingProfileId + anomalyMaxZx10 + activeSensorCount + sensorMv[8]

ACK
- 6 bytes
- fields: packetType, deviceId, seq, ackStatus

Health
- 12 bytes
- fields: packetType, deviceId, seq, powerMode, healthFlags, errorCode, batteryMv
```

`CayenneLPP` pada `FieldReference` hanya referensi historis, bukan format akhir.

### 16.4 ACK and Retry Policy

Perubahan penting dari basis `DESIGN_v1_3.md`:

```text
LORA_ENABLE_ACK  : true
LORA_MAX_RETRY   : 3
LORA_ACK_TIMEOUT_MS : 2000
```
...
Alarm event
- kirim segera saat final result = alarm
- ACK wajib
- retry maksimum = 3
```

Pada battery mode, setelah `transmit`, node membuka `receive window` singkat default `2000 ms` sebelum sleep. Fungsi window ini bukan hanya untuk menerima `ACK`, tetapi juga untuk menerima command dari server yang masih relevan sebelum node mengirim `DONE` ke `TPL5110`. Durasi command downlink window secara default mengikuti `LORA_ACK_TIMEOUT_MS = 2000`, kecuali nanti di-override oleh keputusan implementasi final.


### 16.5 Scheduling Policy

Pada external mode, periode inferensi dan periode normal telemetry boleh berbeda.

```text
External mode
- inference berjalan cepat berdasarkan timer firmware
- normal telemetry LoRa berjalan lebih lambat
- alarm dikirim segera tanpa menunggu period normal
```

Pada battery mode, periode bangun ditentukan oleh `TPL5110`, bukan loop scheduler berulang di firmware.

```text
Battery mode
- wake period dikendalikan TPL5110
- satu siklus inference per wake
- setelah transmit, buka receive window default 2000 ms untuk ACK atau command downlink
- setelah receive window selesai, atau setelah retry / error selesai, kirim DONE ke TPL5110
```


### 16.6 Alarm and Normal Send Rules

Keputusan operasional hasil gabungan:

```text
Jika final detection status = normal:
- kirim sesuai period normal telemetry
- tidak perlu ACK
- kegagalan kirim tidak boleh menghentikan inference berikutnya

Jika final detection status = alarm:
- kirim segera
- butuh ACK
- retry sampai 3 kali bila ACK tidak diterima
- alarm lokal tetap aktif walaupun ACK gagal
```

Status yang diperlakukan sebagai alarm minimal mencakup:

- `classified alarm`
- `unknown anomaly`
- `baseline deviation alarm`

### 16.7 Encoding and Flags Direction
Source: BaseDesign.

Representasi compact tetap memakai enum dan bitmask ringan:

```text
Power mode    : external / battery / error
AI class      : compact numeric enum
Final status  : compact numeric enum
Flags         : alarm, warning, anomaly, battery_low, sensor_error, ai_error, lora_error
```

Konversi nilai analog diarahkan ke format ringkas:

```text
confidence -> 0..100
voltage    -> millivolts
z-score    -> x10 unsigned compact value
```

### 16.8 Implementation Alignment Note

Section 9 membahas arsitektur LoRa; section 16 mengunci payload, ACK, retry, dan scheduling final.
...
Bagian berikut masih perlu dikunci saat implementasi final:

```text
1. Final LoRa frequency / exact field deployment profile bila berbeda dari base draft
2. Apakah moving average tetap aktif di firmware final atau hanya di jalur tertentu
...
6. Final migration rule antara payload compact baru dan referensi historis CayenneLPP
```
