...
const DeviceMap = dynamic(() => import('./DeviceMap').then((m) => m.DeviceMap), {
  ssr: false,
  loading: () => (
    <div style={{ height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--card-bg)', borderRadius: 12, border: '1px solid var(--card-border)', color: 'var(--t4)', fontSize: 13, fontFamily: "'Geist Mono', monospace" }}>
...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ name: string; value: number; color?: string }>; label?: string }) {
  ...
  return (
    <div style={{ background: 'rgba(15,23,42,0.95)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 8, padding: '8px 12px', backdropFilter: 'blur(12px)' }}>
      <p style={{ color: 'var(--t3)', fontSize: 11, marginBottom: 4, fontFamily: "'Geist Mono', monospace" }}>{label}</p>
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color || '#22d3ee', fontSize: 12, margin: '2px 0', fontFamily: "'Geist Mono', monospace" }}>
...
function LiveClock() {
  ...
  return (
    <span suppressHydrationWarning style={{ fontFamily: "'Geist Mono', monospace", fontSize: 13, color: 'var(--t3)', letterSpacing: 0.5 }}>
      {timeText} WIB
    </span>
  );
}
...
export default function GasLeakDashboard() {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--page-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ color: 'var(--t3)', fontSize: 13, fontFamily: "'Geist Mono', monospace" }}>RESTORING SESSION...</div>
        </div>
      </div>
    );
  }
  ...
  if (!currentUser) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--page-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ width: '100%', maxWidth: '380px', padding: 36, background: 'var(--header-bg)', backdropFilter: 'blur(20px)', border: '1px solid var(--card-border)', borderRadius: 24, zIndex: 10, boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            ...
            <p style={{ color: '#94a3b8', fontSize: 13, marginTop: 4, fontFamily: "'Geist Mono', monospace" }}>OPERATIONAL CONTROL PORTAL</p>
          </div>
          ...
          <div style={{ marginTop: 28, textAlign: 'center', fontSize: 11, color: '#475569', fontFamily: "'Geist Mono', monospace" }}>RESTRICTED ACCESS — SYSTEM LOGS ACTIVE</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--page-bg)', color: 'var(--t2)', fontFamily: "'DM Sans', system-ui, sans-serif", display: 'flex', position: 'relative', overflow: 'hidden', WebkitFontSmoothing: 'antialiased' }}>
      <style>{`
      ...
      <nav style={{ width: sidebarCollapsed ? 70 : 240, background: 'var(--sidebar-bg)', borderRight: '1px solid var(--sidebar-border)', display: 'flex', flexDirection: 'column', transition: 'width 0.4s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 50, backdropFilter: 'blur(20px)', position: 'relative' }}>
        <div style={{ padding: '20px 16px', borderBottom: '1px solid var(--divider)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {!sidebarCollapsed && <div><div style={{ fontWeight: 700, fontSize: 15, color: 'var(--t1)', letterSpacing: -0.5 }}>GLD System</div><div style={{ fontSize: 9, color: '#38bdf8', letterSpacing: 1.5, fontWeight: 500, fontFamily: "'Geist Mono', monospace" }}>MULTI-RU</div></div>}
        </div>
          ...
          {SIDEBAR_ITEMS.map((item) => {
            ...
            return (
              <div key={item.key} onClick={() => setTab(item.key)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 8, cursor: 'pointer', transition: 'all 0.2s', background: active ? 'rgba(56, 189, 248, 0.08)' : 'transparent', color: active ? '#22d3ee' : 'var(--t4)', position: 'relative' }}>
                ...
                {item.key === 'alerts' && totalAlerts > 0 && <span style={{ position: sidebarCollapsed ? 'absolute' : 'relative', top: sidebarCollapsed ? 4 : 'auto', right: sidebarCollapsed ? 8 : 'auto', marginLeft: sidebarCollapsed ? 0 : 'auto', background: '#ef4444', color: '#fff', fontSize: 9, fontWeight: 700, padding: '1px 4px', borderRadius: 8, fontFamily: "'Geist Mono', monospace" }}>{totalAlerts}</span>}
              </div>
            );
          })}
        </div>
        <div style={{ padding: '16px 10px', borderTop: '1px solid var(--divider)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px' }}>
            ...
            {!sidebarCollapsed && <div style={{ overflow: 'hidden' }}><div style={{ fontSize: 12, fontWeight: 600, color: 'var(--t2)' }}>Admin</div><div style={{ fontSize: 10, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>{currentUser.name || 'Global'}</div></div>}
          </div>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, zIndex: 10, position: 'relative' }}>
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 24px', background: 'var(--header-bg)', borderBottom: '1px solid var(--divider)', backdropFilter: 'blur(12px)' }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--t3)', marginTop: 1, fontFamily: "'Geist Mono', monospace" }}>Gas Leak Detection Monitoring</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 2, background: darkMode ? 'rgba(15, 23, 42, 0.4)' : 'rgba(255, 255, 255, 0.6)', borderRadius: '10px', padding: '3px', border: '1px solid var(--card-border)', backdropFilter: 'blur(8px)' }}>
              {RU_LIST.map((ru) => (
                <button key={ru} onClick={() => setActiveRU(ru)} style={{ fontFamily: "'Geist Mono', monospace", fontSize: 11, fontWeight: activeRU === ru ? 700 : 500, padding: '5px 10px', border: 'none', borderRadius: '6px', cursor: 'pointer', background: activeRU === ru ? 'linear-gradient(135deg, #0e7490, #0284c7)' : 'transparent', color: activeRU === ru ? '#fff' : 'var(--t3)', textTransform: 'uppercase' }}>{ru}</button>
              ))}
            </div>
            <button onClick={() => setDarkMode(!darkMode)} style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 6, padding: '5px 8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, color: 'var(--t3)', fontSize: 11, fontFamily: "'Geist Mono', monospace" }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ animation: 'fadeIn 0.5s ease', display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div>
                <div style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>Interactive physical positioning</div>
                </div>
                  ...
                  <div className="flex-1 min-h-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={timelineData}>
                        ...
                        <XAxis dataKey="time" tick={{ fill: 'var(--t4)', fontSize: 10, fontFamily: "'Geist Mono', monospace" }} axisLine={false} tickLine={false} interval={3} />
          ...
          {tab === 'alerts' && (
            <div style={{ animation: 'fadeIn 0.5s ease' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16 }}>
                <div><h2 style={{ fontWeight: 600, fontSize: 20, color: 'var(--t1)' }}>Alarm Console</h2><p style={{ fontSize: 13, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>Active: {localAlerts.filter(a => a.status !== 'RESOLVED').length}</p></div>
              </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <span style={{ fontSize: 10, fontFamily: "'Geist Mono', monospace", color: '#60a5fa', background: 'rgba(96,165,250,0.1)', padding: '1px 6px', borderRadius: 4 }}>{a.type}</span>
                          <span style={{ fontSize: 11, color: 'var(--t4)', fontFamily: "'Geist Mono', monospace", display: 'flex', alignItems: 'center', gap: 4 }}>
                            <Clock size={10} />
                            {(() => {
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 12, padding: '18px 20px', backdropFilter: 'blur(12px)' }}>
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 15, color: 'var(--t1)' }}>Weekly Concentration Trend</div>
                    <div style={{ fontSize: 11, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>Average gas levels (PPM) over 7 days</div>
                  </div>
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={analytics?.weeklyTrends || []}>
                      ...
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(56,189,248,0.04)" vertical={false} />
                      <XAxis dataKey="day" tick={{ fill: 'var(--t4)', fontSize: 11, fontFamily: "'Geist Mono', monospace" }} axisLine={false} tickLine={false} />
                      <YAxis tick={{ fill: 'var(--t4)', fontSize: 11, fontFamily: "'Geist Mono', monospace" }} axisLine={false} tickLine={false} />
                      <Tooltip content={<CustomTooltip />} />
                ...
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 12, padding: '18px 20px', backdropFilter: 'blur(12px)' }}>
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 15, color: 'var(--t1)' }}>Weekly Incident Distribution</div>
                    <div style={{ fontSize: 11, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>Threshold breaches per day</div>
                  </div>
                  <ResponsiveContainer width="100%" height={200}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 12, padding: '18px 20px', backdropFilter: 'blur(12px)', display: 'flex', flexDirection: 'column', height: 240 }}>
                  <div style={{ marginBottom: 12, flexShrink: 0 }}>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 15, color: 'var(--t1)' }}>Site Performance Ranking</div>
                    <div style={{ fontSize: 11, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>Efficiency & Safety Index</div>
                  </div>
                      ...
                      <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                        <tr style={{ background: darkMode ? '#111' : '#fff' }}>
                          {['Site', 'Uptime', 'Events', 'Safety'].map((h) => (
                            <th key={h} style={{ textAlign: 'left', padding: '8px', fontSize: 11, color: 'var(--t3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, fontFamily: "'Geist Mono', monospace", borderBottom: '1px solid var(--divider)' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {analytics?.siteRankings.map((s, i) => (
                          <tr key={s.ru} style={{ background: i % 2 === 0 ? 'var(--card-bg-alt)' : 'transparent' }}>
                            <td style={{ padding: '8px', fontWeight: 700, color: '#38bdf8', fontFamily: "'Geist Mono', monospace", fontSize: 12 }}>{s.ru}</td>
                            <td style={{ padding: '8px', fontSize: 12, fontWeight: 600, color: 'var(--t2)', fontFamily: "'Geist Mono', monospace" }}>{s.uptime.toFixed(1)}%</td>
                ...
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 12, padding: '18px 20px', backdropFilter: 'blur(12px)', height: 240 }}>
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 15, color: 'var(--t1)' }}>Connectivity Grade</div>
                    <div style={{ fontSize: 11, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>Network quality distribution</div>
                  </div>
                  <div style={{ height: 160 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={networkQuality} layout="vertical" margin={{ left: -20 }}>
                        ...
                        <YAxis dataKey="grade" type="category" tick={{ fill: 'var(--t4)', fontSize: 11, fontFamily: "'Geist Mono', monospace" }} axisLine={false} tickLine={false} />
                        <Tooltip content={<CustomTooltip />} />
            ...
            <div style={{ animation: 'fadeIn 0.5s ease', maxWidth: '800px', display: 'flex', flexDirection: 'column', gap: 24, margin: '0 auto', width: '100%' }}>
              <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 14, padding: '24px 28px', backdropFilter: 'blur(12px)' }}>
                <div style={{ marginBottom: 20 }}>
                  <h2 style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 24, color: 'var(--t1)' }}>System Configuration</h2>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
                      <div style={{ background: 'var(--card-bg-alt)', padding: 16, borderRadius: 12, border: '1px solid var(--divider)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                          ...
                          <span style={{ fontSize: 16, fontWeight: 700, color: '#f59e0b', fontFamily: "'Geist Mono', monospace" }}>{sysSettings?.warningThreshold} PPM</span>
                        </div>
                        <input 
                      ...
                      <div style={{ background: 'var(--card-bg-alt)', padding: 16, borderRadius: 12, border: '1px solid var(--divider)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                          ...
                          <span style={{ fontSize: 16, fontWeight: 700, color: '#ef4444', fontFamily: "'Geist Mono', monospace" }}>{sysSettings?.criticalThreshold} PPM</span>
                        </div>
                        <input 
              ...
              <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 14, padding: '20px 22px', backdropFilter: 'blur(12px)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <div>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 24, color: 'var(--t1)' }}>User Management</div>
                    <div style={{ fontSize: 14, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace", marginTop: 2 }}>
                    ...
                    <thead>
                      <tr>
                        {['User', 'RU Access', 'Role', 'Created', 'Actions'].map((h) => (
                          <th key={h} style={{ textAlign: 'left', padding: '10px', fontSize: 12, color: 'var(--t3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, fontFamily: "'Geist Mono', monospace", borderBottom: '1px solid var(--divider)' }}>
                            {h}
                          </th>
                        ))}
                      </tr>
                          ...
                          <td style={{ padding: '12px 10px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              <div>
                                ...
                                <div style={{ fontSize: 12, color: 'var(--t4)', fontFamily: "'Geist Mono', monospace" }}>{u.email}</div>
                              </div>
                            </div>
                          </td>
                          <td style={{ padding: '12px 10px' }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: '#38bdf8', background: 'rgba(56,189,248,0.1)', padding: '2px 6px', borderRadius: 4, fontFamily: "'Geist Mono', monospace" }}>{u.ruId}</span>
                          </td>
                          <td style={{ padding: '12px 10px', fontSize: 13, color: 'var(--t3)' }}>{u.role}</td>
                          <td style={{ padding: '12px 10px', fontSize: 13, color: 'var(--t4)', fontFamily: "'Geist Mono', monospace" }}>{new Date(u.createdAt).toLocaleDateString()}</td>
                          <td style={{ padding: '12px 10px' }}>
                            <button
              ...
              {showAddUser && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                  <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 16, padding: 28, width: '100%', maxWidth: '400px', animation: 'fadeIn 0.3s ease' }}>
                    <h3 style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 20, color: 'var(--t1)', marginBottom: 20 }}>Add New Administrator</h3>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderTop: '1px solid var(--divider)', marginTop: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><PulseDot color="#22d3ee" size={5} /><span style={{ fontSize: 11, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>System Live</span></div>
              <span style={{ fontSize: 11, color: 'var(--t4)', fontFamily: "'Geist Mono', monospace" }}>|</span>
              <span style={{ fontSize: 11, color: 'var(--t3)', fontFamily: "'Geist Mono', monospace" }}>v.2.1 Stable</span>
            </div>
          </div>
        </div>
      </main>
      ...
      {showRenameModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 16, padding: 28, width: '100%', maxWidth: '400px', animation: 'fadeIn 0.3s ease' }}>
            <h3 style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 20, color: 'var(--t1)', marginBottom: 8 }}>Rename Device</h3>
            <p style={{ fontSize: 13, color: 'var(--t3)', marginBottom: 20 }}>Modify display name for <span style={{ fontFamily: "'Geist Mono', monospace", color: '#38bdf8' }}>{deviceToRename?.macAddress}</span></p>
